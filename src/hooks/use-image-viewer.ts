...
import ImageInfoPanel from "@/components/media/image-info-panel";
...
export function useImageViewer(initialOptions: ViewerProOptions = {}) {
  const [images, setImages] = useState<ViewerItem[]>(
  ...
  // 自定义信息渲染节点
  const createCustomInfoNode = useCallback(
    (viewerItem: ViewerItem, idx: number) => {
      // 清理之前的容器（如果存在）
      ...
      root.render(createElement(ImageInfoPanel, { index: idx, viewerItem }));
  ...
  const applyImageFrameSize = useCallback(
    (frame: HTMLElement, imgObj: ViewerItem) => {
  ...
  // 自定义渲染节点
  const createCustomRenderNode = useCallback(
    (imgObj: ViewerItem, idx: number) => {
      ...
      const createEosImage = (src: string) => {
        ...
        const imageWithMetadata = imgObj as ViewerItem & {
          blurhash?: string;
          title?: string;
        };
  ...
  const handleTransformChange = useCallback((state: TransformChangeState) => {
    ...
    const contentLayer = document.getElementById(
  ...
  const initViewer = useCallback(() => {
    ...
    const viewerOptions: ViewerProOptions = {
      ...
      mobileToolbar: initialOptions.mobileToolbar ?? [
        "zoomIn",
        "zoomOut",
        "reset",
        "thumbnails",
      ...
      onImageLoad:
        initialOptions.onImageLoad ||
        ((_imgObj: ViewerItem, _idx: number) => undefined),
      // 自定义渲染节点时必须同步手势变换
      onTransformChange:
        initialOptions.onTransformChange || handleTransformChange,
  ...
  const convertPostImagesToImageObj = useCallback(
    (postImages: PostImage[]): ViewerItem[] => {
