...
export const useCollectTopic = () => {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      isCollected
        ? collectionsApi.delete(topicId)
        : collectionsApi.create(topicId),
    onError: (err) => toast.error("操作失败", { description: err.message }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: queryKeys.topics.all });
...
export function usePostActions(
  options: { debounceMs?: number; onUnauthorized?: () => void } = {}
) {
  ...
  const debouncedLike = useMemo(
    () =>
      debounce((topicId: string, isLiked: boolean) => {
        likeRef.current.mutate({ isLiked, topicId });
      }, debounceMs),
    [debounceMs]
  );
  ...
  const debouncedCollect = useMemo(
    () =>
      debounce((topicId: string, isSaved: boolean) => {
        collectRef.current.mutate({ isCollected: isSaved, topicId });
      }, debounceMs),
    [debounceMs]
  );
