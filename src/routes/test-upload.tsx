...
import { CheckCircle2, FileImage, LoaderCircle, MapPin, Upload, X } from "lucide-react";
import { type ChangeEvent, type DragEvent, useMemo, useState } from "react";
import { useFileUpload } from "@/hooks/useFileUpload";
import type { OSSFile } from "@/services/upload";
import { extractGPSFromImage, type GPSCoordinates } from "@/utils/upload/gpsExtractor";
...
function TestUploadPage() {
  ...
  const locationMap = useMemo(() => {
    ...
    for (const item of selectedFiles) {
      if (item.gps) map.set(item.file, item.gps);
    }
    return map;
  }, [selectedFiles]);
  ...
  const addFiles = async (files: File[]) => {
    ...
    const gpsResults = await Promise.all(
      entries.map((entry) =>
        entry.readingGPS ? extractGPSFromImage(entry.file) : Promise.resolve(null)
      )
    );
    setSelectedFiles((current) =>
      current.map((item) => {
        const index = entries.findIndex((entry) => entry.id === item.id);
        return index === -1 ? item : { ...item, gps: gpsResults[index], readingGPS: false };
      })
    );
  };
  ...
  const isComplete = uploadState.stage === "complete" && !uploadState.isUploading;
    ...
    <main className="min-h-screen bg-[#f6f7f8] px-4 py-8 text-[#1f2933] sm:px-8">
      <div className="mx-auto max-w-4xl">
        <header className="border-[#d9dee3] border-b pb-5">
          ...
          <h1 className="mt-2 font-semibold text-2xl tracking-tight">上传组件测试</h1>
          ...
          <div className="p-5">
            ...
            >
              <input
                ...
                type="file"
              />
              <Upload aria-hidden="true" className="h-6 w-6 text-[#64748b]" strokeWidth={1.7} />
              <span className="mt-3 font-medium text-sm">点击选择文件，或将文件拖到这里</span>
              <span className="mt-1 text-[#64748b] text-xs">文件只会先在浏览器读取元数据</span>
            </label>
        ...
        >
          <div className="flex flex-col gap-4 border-[#e5e7eb] border-b px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            >
              ...
              {uploadState.isUploading ? "上传中…" : isComplete ? "已完成" : "开始上传"}
            </button>
          </div>
            ...
            <div className="m-5 rounded-md border border-[#b7dec8] bg-[#f2fbf5] p-4">
              ...
              {uploadedFiles.map((file) => (
                <div className="mt-3 flex gap-2 text-sm" key={file.sec_uid}>
                  <MapPin aria-hidden="true" className="mt-0.5 h-4 w-4 shrink-0 text-[#166534]" />
                  <div>
...
export const Route = createFileRoute("/test-upload")({ component: TestUploadPage });
