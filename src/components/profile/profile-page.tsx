...
/* biome-ignore-all lint/suspicious/noShadowRestrictedNames: this existing integration requires the current implementation */
...
/* biome-ignore-all lint/performance/noJsxPropsBind: profile actions intentionally capture page state */
...
import SimpleInfiniteScroll from "@/components/common/simple-infinite-scroll";
import { GithubIcon as Github } from "@/components/icons/github-icon";

const PROTOCOL_PATTERN = /^https?:\/\//;

import LottieAnimation from "@/components/lottie/lottie-animation";
import TrackPage from "@/components/map/track-page";
import SimpleImageDetail from "@/components/media/simple-image-detail";
import CreatePostModal from "@/components/post/create-post-modal";
import PhotoWall from "@/components/profile/photo-wall";
...
import { useCollectTopic, useLikeTopic } from "@/hooks/use-interactions";
import {
...
export default function ProfilePage({
  ...
  const handleTabChange = (value: string) => {
    ...
    navigate({
      replace: true,
      search: { tab: tab === "posts" ? undefined : tab, userId: propUserId },
      to: "/profile",
    });
  };
  ...
  // 获取目标用户信息（如果查看其他用户）
  const { data: targetUserData, isLoading: isLoadingTargetUser } =
    useUserById(propUserId);
  ...
  const tabs = [
    ...
    {
      count: userStats?.photo_count,
      icon: Image,
      id: "photos",
      label: "照片墙",
    },
    { count: userStats?.track_count, icon: Map, id: "track", label: "轨迹" },
    {
      ...
      <Card className="group relative overflow-hidden border-border/50 transition-all hover:border-border">
        ...
        <CardContent className="relative px-8 pb-8">
          ...
          <div
            ...
            onClick={
              isViewingOwnProfile ? () => setIsEditDialogOpen(true) : undefined
            }
          >
            {/* 外层装饰框 */}
              ...
              {!!displayUser.website && (
                ...
                >
                  ...
                  <span className="hidden sm:inline">
                    {displayUser.website.replace(PROTOCOL_PATTERN, "")}
                  </span>
                </a>
              )}
          ...
          <div className="flex items-center gap-8">
            {stats.map((stat, index) => (
              <div className="group/stat relative" key={stat.label}>
            ...
            <CardContent className="p-6">
              {isLoadingTopics && posts.length === 0 ? (
                ...
                <LottieAnimation
                  ...
                  emptyDescription={
                    isViewingOwnProfile ? "记录并分享你的精彩瞬间" : ""
                  }
                  emptyTitle={
                    isViewingOwnProfile ? "还没有发布内容" : "TA还没有发布内容"
                  }
                  height={200}
                  type="empty"
                  width={200}
          ...
          onSubmit={async (postData: any) => {
            try {
              ...
              const originalData = {
                ...
                file_sec_uids:
                  editingTopicData.images?.map((img) => img.sec_uid) || [],
                tags:
