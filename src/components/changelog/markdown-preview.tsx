...
const components: Components = {
  ...
  h1: ({ children, ...props }) => {
    ...
    return (
      <h1
        className="mt-6 mb-4 text-center font-bold text-2xl text-foreground"
        id={id}
        {...props}
      >
        {children}
  ...
  hr: ({ ...props }) => (
    <hr className="my-8 border-border/60 border-t" {...props} />
  ),
  li: ({ children, ...props }) => (
    <li
  ...
  td: ({ children, ...props }) => (
    <td
      className="border-border/50 border-b px-4 py-3 text-muted-foreground"
      {...props}
    >
      {children}
    </td>
