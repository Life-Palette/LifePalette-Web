...
import { type AIAction, AIToolbar } from "@/components/editor/ai-toolbar";
import { GhostText } from "@/components/editor/ghost-text";
...
const DEFAULT_VALUE: Value = [{ children: [{ text: "" }], type: "p" }];
...
export function PlateEditor({
  ...
  const handleEditorChange = useCallback(
    ({ value: editorValue }: { value: Value }) => onChange(editorValue),
    [onChange]
  );
    ...
    <Plate editor={editor} onChange={handleEditorChange}>
      ...
      >
        ...
        <EditorContainer className="relative w-full border-0 bg-transparent">
          ...
          {!!suggestion && !!enableCompletion && (
            <GhostText suggestion={suggestion} />
          )}
        </EditorContainer>
      </div>
    </Plate>
