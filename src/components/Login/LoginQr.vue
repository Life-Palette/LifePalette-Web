<template>
  <div class="login-content-code flex-col flex justify-center items-center">
    <div class="desc-title">扫码登录</div>
    <div class="desc-subtitle">请使用<span>移动端app</span>扫描二维码</div>
    <div class="qr-code-area relative h-[300px] w-[300px]">
      ...
      /> -->
      <div class="qr-cover">
        <div class="qr-coner-1"></div>
        <div class="qr-coner-2"></div>
        <div class="qr-coner-3"></div>
        <div class="qr-coner-4"></div>
        <el-image
          @click="handle_refreshLoginQrCode"
          class="w-full h-full"
          :src="qrImg"
      ...
      <template v-if="qrCodeStatus === 'confirm'">
        ...
        >
          <!-- 遮罩层 -->
          <div
            class="absolute top-0 left-0 w-full h-full bg-white opacity-80 z-98"
          ></div>
          <!-- 标识 -->
          <div
            class="i-carbon-checkmark-filled color-[#36bf84] text-4xl z-99"
          ></div>
        </div>
      </template>
      <!-- 过期遮罩层 -->
      <template v-else-if="qrCodeStatus === 'timeout'">
        <div
          @click="handle_refreshLoginQrCode"
          class="w-full cursor-pointer h-full absolute top-0 left-0 flex items-center justify-center backdrop-filter backdrop-blur-sm"
        >
          <!-- 遮罩层 -->
          <div
            class="absolute top-0 left-0 w-full h-full bg-black opacity-50 z-98 "
          ></div>
          <!-- 标识 -->
          <div
            class="i-carbon-warning-filled color-[#ff4d4f] text-4xl z-99"
          ></div>
          <!-- 刷新 -->
          <div
            class="absolute flex items-center justify-center bottom-10 color-white w-full z-99"
          >
            点击刷新二维码
          </div>
        </div>
      ...
      <template v-else-if="qrCodeStatus === 'success'">
        ...
        >
          ...
          <!-- 标识 -->
          <div
            class="i-carbon-face-wink-filled color-[#36bf84] text-4xl z-99"
          ></div>
        </div>
      </template>
    </div>
