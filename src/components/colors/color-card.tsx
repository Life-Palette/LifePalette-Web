...
export default function ColorCard({
  ...
  if (viewMode === "list") {
    return (
      ...
      >
        <button
          className="flex min-w-0 flex-1 items-center gap-4 bg-transparent p-0 text-left"
          onClick={onClick}
          type="button"
        >
  ...
  return (
    <div
      className={cn(
        ...
        sizeClasses[size],
        isSelected &&
          "scale-[1.03] ring-2 ring-primary ring-offset-2 ring-offset-background"
      )}
    >
      <button
        className="absolute inset-0 m-0 block border-0 bg-transparent p-0 text-left"
        onClick={onClick}
        style={{ backgroundColor: color.hex }}
        type="button"
      >
      ...
      <Button
        className={cn(
          "absolute right-3 bottom-3 z-20 h-8 w-8 rounded-lg transition-all",
          luminance > 0.5
      ...
      <div
        className={cn(
          "absolute bottom-2.5 left-3 z-10 transition-opacity group-hover:opacity-0"
        )}
      >
        <span
          className={cn(
            "font-mono font-semibold text-xs tracking-wide",
            textColor
          )}
        >
          {color.hex.toUpperCase()}
