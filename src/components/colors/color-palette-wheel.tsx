...
export default function ColorPaletteWheel({
  ...
  const centerTextColor =
    centerLuminance > 0.5 ? "text-black/80" : "text-white/90";

  const handlePaletteClick = useCallback(
    (event: MouseEvent<HTMLButtonElement>) => {
      const color = paletteColors.find(
        (item) => item.id === event.currentTarget.dataset.colorId
      );
      if (color) {
        onColorClick(color);
      }
    },
    [onColorClick, paletteColors]
  );
      ...
      >
        ...
        {!!centerColor && (
          ...
          >
            <span
              className={cn(
                "font-bold font-mono text-[13px] tracking-wider",
                centerTextColor
              )}
            >
