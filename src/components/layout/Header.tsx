...
export default function Header({ activeTab, onLogin, onTabChange }: HeaderProps) {
  ...
  return (
    <header className="fixed top-0 right-0 left-0 z-50 border-border/40 border-b bg-background/80 backdrop-blur-xl transition-all duration-300 dark:border-white/5 dark:bg-background/80">
          ...
          <div className="mx-4 hidden max-w-lg flex-1 sm:block md:mx-8">
            <Link className="block" preload="intent" to="/search">
              <div className="group relative">
                <Search
                  className="pointer-events-none absolute top-1/2 left-4 -translate-y-1/2 transform text-muted-foreground transition-colors group-focus-within:text-primary"
                  size={18}
                />
                <Input
                  className="w-full rounded-full border-transparent bg-secondary/50 py-3 pr-4 pl-12 text-sm shadow-sm transition-all hover:bg-secondary/80 hover:shadow-md focus:border-primary/20 focus:bg-background focus:ring-2 focus:ring-primary/20 dark:border-white/5 dark:bg-white/10 dark:focus:bg-black/40 dark:hover:bg-white/15"
                  placeholder="搜索有趣的内容..."
                  readOnly
                  type="text"
                />
          ...
          <div className="flex items-center gap-3">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                ...
                >
                  <Button
                    className="h-9 w-9 rounded-full border border-border/50 bg-secondary/50 text-muted-foreground transition-all duration-200 hover:border-primary/30 hover:bg-secondary hover:text-foreground dark:bg-white/5 dark:hover:bg-white/10"
                    size="icon"
                    variant="ghost"
                  >
                    <Github size={18} />
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Link preload="intent" to="/changelog">
                  <Button
                    className="relative h-9 w-9 rounded-full border border-border/50 bg-secondary/50 text-muted-foreground transition-all duration-200 hover:border-primary/30 hover:bg-secondary hover:text-foreground dark:bg-white/5 dark:hover:bg-white/10"
                    size="icon"
                    variant="ghost"
                  >
                    <History size={18} />
