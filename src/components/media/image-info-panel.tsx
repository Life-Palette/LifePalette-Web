...
export default function ImageInfoPanel({
    ...
    try {
      ...
      mapRef.current.on("load", () => {
        ...
        new mapboxgl.Marker({ color: "#60a5fa" })
          .setLngLat([detail.lng ?? 0, detail.lat ?? 0])
          .addTo(map);
      });
    } catch (error: unknown) {
  ...
  const device = useMemo(() => {
    ...
    return (
      [detail.deviceMake, detail.deviceModel].filter(Boolean).join(" ") || null
    );
  }, [detail]);
  ...
  if (!detail) {
    return (
      <div className="p-6 text-sm text-white/50">
        {viewerItem.title || "未命名"}
      </div>
    );
  }

  const hasParams =
    detail.fNumber || detail.exposureTime || detail.iso || detail.focalLength;
      ...
      <div className="space-y-5 p-5 pb-6">
        ...
        {!!device && (
          <div>
            ...
            {!!detail.lensModel && (
              <div className="pl-6 text-slate-400 text-xs">
                {detail.lensModel}
              </div>
            )}
          </div>
            ...
            <div className="flex gap-1 overflow-hidden rounded-lg">
              {sortedColors.map((c) => (
                <div
                  ...
                  key={`${c.rank}-${c.color?.hex ?? "unknown"}`}
