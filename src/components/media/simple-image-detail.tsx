...
/* biome-ignore-all lint/performance/noJsxPropsBind: this existing integration requires the current implementation */
...
/* biome-ignore-all lint/suspicious/useAwait: this existing integration requires the current implementation */
/* biome-ignore-all lint/style/noNestedTernary: this existing integration requires the current implementation */
/* biome-ignore-all lint/a11y/useKeyWithClickEvents: this existing integration requires the current implementation */
/* biome-ignore-all lint/a11y/noStaticElementInteractions: this existing integration requires the current implementation */
...
/* biome-ignore-all lint/suspicious/noShadow: this existing integration requires the current implementation */
/* biome-ignore-all lint/security/noDangerouslySetInnerHtml: this existing integration requires the current implementation */
...
import type { MediaCarouselRef } from "@/components/media/media-carousel";
import MediaCarousel from "@/components/media/media-carousel";
import CommentSection from "@/components/post/comment-section";
import { Button } from "@/components/ui/button";
import { ScrollArea } from "@/components/ui/scroll-area";
import { useIsAuthenticated } from "@/hooks/use-auth";
import { usePostActions } from "@/hooks/use-interactions";
import {
  useComments,
  useCreateComment,
  useDeleteTopic,
  useTopicDetail,
} from "@/hooks/use-topic-detail";
...
export default function SimpleImageDetail({
  ...
  }, [isAuthenticated, postActions, topic, topicId]);
  ...
  const handleSubmitComment = useCallback(
    ...
    [createCommentMutation, isAuthenticated, topicId, user]
  );

  const handleSubmitReply = useCallback(
    (commentId: number, content: string, _replyToUserId?: number) => {
      if (!(content.trim() && isAuthenticated && user)) {
        return;
      }
  ...
  }, [deleteTopicMutation, onClose, topicId]);
            ...
            >
              ...
              >
                ...
                {isLoading ? (
                  ...
                  <div
                    className={`relative flex w-full ${hasImages ? "h-full flex-col overflow-hidden lg:flex-row" : "max-h-[85vh] flex-col overflow-hidden"}`}
                  >
                    {/* 左侧：图片区域 */}
                    {!!hasImages && (
                    ...
                    <motion.div
                      ...
                      className={`relative flex w-full shrink-0 flex-col bg-white ${
                        hasImages
                          ? "min-h-0 flex-1 lg:h-full lg:w-[450px] lg:flex-none"
                          : "max-h-[85vh] w-full"
                      }`}
                    ...
                    >
                      ...
                      <div className="pointer-events-none absolute inset-0">
                        ...
                        <svg
                          aria-hidden="true"
                          className="absolute top-0 left-0 h-32 w-full opacity-[0.02]"
                        >
                          <defs>
                            <pattern
                              height="20"
                          ...
                          <div className="flex items-baseline justify-between gap-4">
                            ...
                            <div className="flex flex-shrink-0 flex-col items-end gap-1">
                              ...
                              {topic.updatedAt &&
                                ...
                                Math.floor(
                                  new Date(topic.updatedAt).getTime() / 60_000
                                ) !==
                                  Math.floor(
                                    new Date(topic.createdAt).getTime() / 60_000
                                  ) && (
                      ...
                      {/* 中间：可滚动内容区域 */}
                      <ScrollArea
                        className={`min-h-0 flex-1 overflow-auto ${!hasImages && "min-h-[300px]"}`}
                      >
