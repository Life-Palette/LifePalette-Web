...
const MapCard: React.FC<MapCardProps> = ({ data = {}, isDark = false }) => {
  ...
  const addMarker = useCallback(
    (lnglat: number[], markerData?: MapData, isSingle?: boolean) => {
      if (!map.current) {
        return;
      }
      ...
      if (markerData) {
        ...
        if (isSingle) {
          const { id } = markerData;
          cover = getCover(markerData) || {};
          dot.className = `marker-dot-${id} marker-dot`;
        } else {
          const { fileList, id } = markerData;
  ...
  const initMap = useCallback(() => {
    ...
    try {
      ...
      // Ignore map initialization errors and let the cleanup below run.
    }
