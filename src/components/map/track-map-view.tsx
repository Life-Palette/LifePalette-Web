...
const CONVERTIBLE_IMAGE_PATTERN = /\.(heic|heif|tiff?)$/i;
...
const createPhotoMarkers = (
  ...
  for (const feature of features) {
    ...
    const fileData = filteredFiles.find(
      (file) => file.url === feature.properties.url
    );
    ...
    markerData.push({ coords, el, imgSrc, loaded: false, marker });
...
const TrackMapView: React.FC<TrackMapViewProps> = ({
  ...
  const eventHandlers = useRef<
    { event: mapboxgl.MapEventType; handler: MapEventHandler; layer?: string }[]
  >([]);
  ...
  const [previewImages, setPreviewImages] = useState<PostImage[]>([]);
  ...
  const cleanupEventHandlers = useCallback(() => {
    ...
    for (const { event, handler, layer } of eventHandlers.current) {
      try {
        if (layer) {
          map.current.off(event, layer, handler);
        } else {
  ...
  const openImagePreview = useCallback((fileData: FileData) => {
    const needsFormatConversion =
      ...
      CONVERTIBLE_IMAGE_PATTERN.test(fileData.name);
  ...
  const renderPopup = useCallback(
    (coords: [number, number], fileData: FileData, index?: number) => {
      ...
      const popupImgUrl = getThumbnailUrl(
        fileData.url,
        fileData.width,
        fileData.height,
        400
      );
      const photoIndex = filteredFiles.findIndex(
        (f) => f.sec_uid === fileData.sec_uid
      );
      ...
      const device = [fileData.deviceMake, fileData.deviceModel]
        .filter(Boolean)
        .join(" ");
      const paramParts = [
  ...
  const showPhotoPopup = useCallback(
    (coords: [number, number], fileData: FileData, index?: number) => {
      const fileSecUid = fileData.sec_uid;

      // 如果缓存中有详情，直接用
      const cached = fileDetailCache.current.get(fileSecUid);
      if (cached) {
      ...
      filesApi
        ...
        .then((res) => {
          ...
          fileDetailCache.current.set(fileSecUid, enriched);
          if (currentPopup.current) {
  ...
  const addPhotoLayers = useCallback(() => {
    ...
    if (map.current.getSource("points")) {
      for (const layerId of Object.values(LAYER_NAMES)) {
    ...
    const markerData = createPhotoMarkers(
    ...
    const updateMarkersVisibility = () => {
      ...
      // 更新各 marker
      updatePhotoMarkers(map.current, markerData, showMarkers, bounds);
    };
    ...
    const pointClickHandler = (e: mapboxgl.MapMouseEvent) => {
      ...
      const coords = (geometry as GeoJSON.Point).coordinates.slice() as [
        number,
        number,
      ];
      if (!properties) {
    ...
    if (autoFitBounds && pointsGeoJSON.features.length > 0) {
      ...
      for (const point of pointsGeoJSON.features) {
        bounds.extend(
      ...
      if (map.current) {
        ...
        if (canvas && canvasMouseMoveHandler.current) {
          canvas.removeEventListener(
            "mousemove",
            canvasMouseMoveHandler.current
          );
        }
        map.current.remove();
        map.current = null;
  ...
  const clearSelection = useCallback(() => setSelectedPhotos([]), []);
  ...
  return (
    <div
      ...
      style={{
        height: "100%",
        minHeight: minHeight ?? (isMobile ? "100dvh" : "600px"),
      }}
    >
      ...
      <button
        ...
        style={{
          right: showGallery && !isMobile ? `${GALLERY_WIDTH}px` : undefined,
        }}
