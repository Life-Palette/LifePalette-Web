...
const ExportMapDialog: React.FC<ExportMapDialogProps> = ({
  ...
  const mapReadyHandlerRef = useRef<
    ((readyMap: MapboxMap | null) => void) | null
  >(null);

  const handleMapReady = useCallback((readyMap: MapboxMap | null) => {
    mapReadyHandlerRef.current?.(readyMap);
  }, []);

  const createExportContainer = useCallback(() => {
  ...
  const buildMapImage = useCallback(
    (mapInstance: MapboxMap, scale: number) => {
      ...
      if (watermarkConfig.enabled && watermarkConfig.text) {
        ...
        context.fillText(
          ...
          boxX + paddingX,
          boxY + boxHeight / 2 + paddingY / 8
        );
      }

      return outputCanvas.toDataURL("image/png", 1);
    },
    [exportConfig.height, exportConfig.width, watermarkConfig]
  );

  const renderExportMap = useCallback(
    async (scale: number) => {
      const exportContainer = createExportContainer();
      const editorMapWidth =
        editorMapFrameRef.current?.clientWidth || exportConfig.width;
      const exportZoom =
        mapViewRef.current.zoom +
        Math.log2(exportConfig.width / editorMapWidth);
      ...
      try {
            ...
            <QueryClientProvider client={queryClient}>
              ...
              >
                <TrackMapView
                  autoFitBounds={false}
                  customCenter={[
                    mapViewRef.current.centerLng,
                    mapViewRef.current.centerLat,
                  ]}
        ...
        mapReadyHandlerRef.current = null;
        root.unmount();
  ...
  const handleWidthChange = useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) =>
      handleSizeChange("width", event.target.value),
    [handleSizeChange]
  );
  const handleHeightChange = useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) =>
      handleSizeChange("height", event.target.value),
    [handleSizeChange]
  );
  ...
  }, [onOpenChange, renderExportMap]);

  const handleWatermarkEnabledChange = useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  }, [handleGeneratePreview, showPreview]);
  const handleCancel = useCallback(() => onOpenChange(false), [onOpenChange]);

  let previewButtonLabel = "预览效果";
  if (showPreview) {
    previewButtonLabel = "返回编辑";
  }
  if (isGeneratingPreview) {
    previewButtonLabel = "生成中...";
  }
      ...
      <DialogContent className="max-h-[90vh] w-[calc(100%-2rem)] overflow-hidden sm:max-w-[calc(100%-2rem)] lg:max-w-5xl">
        ...
        <div className="grid max-h-[calc(90vh-4.5rem)] grid-cols-1 gap-4 overflow-y-auto pr-1 lg:grid-cols-[320px_minmax(0,1fr)] lg:gap-6">
          ...
          <div className="flex min-w-0 flex-col space-y-3">
            <div>
              <div className="mb-2 block font-medium text-sm">尺寸设置</div>
            ...
            <div>
              <div className="mb-2 block font-medium text-sm">当前视图</div>
              <div className="space-y-2">
                ...
                <div className="rounded-lg bg-muted p-3">
                  ...
                  <div className="font-mono text-sm">
                    {displayView.centerLng.toFixed(4)},{" "}
                    {displayView.centerLat.toFixed(4)}
                  </div>
                </div>
              </div>
            ...
            <div>
              <div className="mb-2 block font-medium text-sm">水印设置</div>
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={handleWatermarkEnabledChange}
                    type="checkbox"
                  />
