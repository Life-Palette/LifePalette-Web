...
/* biome-ignore-all lint/performance/noJsxPropsBind: publishing controls intentionally capture form state */
...
import { z } from "zod";
import { UploadOverlay } from "@/components/common/upload-overlay";
...
const initialValue: Value = [{ children: [{ text: "" }], type: "p" }];
const titleSchema = z
  .string()
  .min(1, "标题不能为空")
    ...
    try {
      ...
      if (newItems.length > 0) {
        ...
        newItems.forEach((item) => {
          ...
          if (
            ...
            locationMap.set(item.data.file, {
              lat: item.data.lat,
              lng: item.data.lng,
            });
          }
      ...
      for (const item of mediaItems) {
        if (item.type === "existing") {
          ...
          if (uploadedFiles[offset]) {
            fileSecUids.push(uploadedFiles[offset].sec_uid);
          }
          offset += 1;
          if (item.data.videoFile) {
            offset += 1;
