/* biome-ignore-all lint/complexity/noForEach: this existing integration requires the current implementation */
...
/* biome-ignore-all lint/performance/noJsxPropsBind: palette controls intentionally capture local state */
...
import ColorDetailPanel from "@/components/colors/color-detail-panel";
import ColorPaletteSkeleton from "@/components/colors/color-palette-skeleton";
import ColorPaletteWheel from "@/components/colors/color-palette-wheel";
import type { ColorItem, FileItem } from "@/components/colors/types";
...
export default function ColorPalettePage() {
    ...
    allPages.forEach((page) => {
      ...
      list.forEach((item: any) => {
        const representativeImage = item.file?.url
          ? { blurhash: item.file.blurhash || "", url: item.file.url }
          : undefined;
        ...
        allItems.push({
          ...
          representativeImage,
          rgb: { b: item.b, g: item.g, r: item.r },
        });
      });
    });
  ...
  // 总颜色数和图片数
  const totalColors =
    colorStatsData?.pages?.[0]?.result?.pagination?.total || colors.length;
